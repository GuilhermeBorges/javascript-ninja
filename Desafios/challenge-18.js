...
(function () {
  ...
  */
  ...
  function cleanCPF (cpf) {
    const limpar = /[^0-9]/g
    const construir = /(\d{3})(\d{3})(\d{3})(\d{2})/g
    return cpf
      .replace(limpar, '')
      .replace(construir, function (expressao, p1, p2, p3, p4) {
        return `${p1}.${p2}.${p3}-${p4}`
      })
  }
  /*
  ...
  */
  ...
  function matchHtml (texto) {
    const regex = /<(div|section|blockquote|\w+)>/gi
    return (texto.match(regex))
  }
  texto = '<div><section><blockquote>Texto <img /></blockquote></section></div><a><asdasd>'
  console.log(matchHtml(texto))
  ...
  */
  ...
  function matchEmptyHtml (texto) {
    const regex = /<\w+><\/\w+>/gi
    return texto.match(regex)
  }
  texto = '<div><ul><li></li><li></li><li><span></span></li></ul></div>'
  console.log(matchEmptyHtml(texto))
